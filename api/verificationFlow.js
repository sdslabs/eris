...
export async function handleGetVerifyFlow() {
  try {
    const getResponse = await axiosInstance.get(process.env.NEXT_PUBLIC_BASE_URL + "/verification");
...
export async function handlePostVerifyFlow(flowID, csrf_token, email) {
  try {
    ...
    const res = await axiosInstance.post(process.env.NEXT_PUBLIC_BASE_URL + "/verification", objData);
