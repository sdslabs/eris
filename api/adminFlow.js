...
export async function handleGetIdentitiesFlow() {
  try {
    const getResponse = await axiosInstance.get(process.env.NEXT_PUBLIC_BASE_URL + "");
...
export async function handleBanIdentityFlow(identity) {
  try {
    const res = await axiosInstance.put(process.env.NEXT_PUBLIC_BASE_URL + "", { identity });
    return res;
  } catch (error) {
    throw error;
  }
...
export async function handleDeleteIdentityFlow(identity) {
  try {
    const res = await axiosInstance.post(process.env.NEXT_PUBLIC_BASE_URL + "", { identity });
    return res;
  } catch (error) {
    throw error;
  }
...
export async function handleCreateIdentityFlow(name, phone_number, email, role) {
  try {
    ...
    const res = await axiosInstance.post(process.env.NEXT_PUBLIC_BASE_URL + "", objData);
    return res;
  } catch (error) {
    throw error;
  }
