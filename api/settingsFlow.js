...
export async function handlePostToggleTOTPFlow(flowID, csrf_token, totp_code, totp_unlink) {
  try {
    ...
    const res = await axiosInstance.post(process.env.NEXT_PUBLIC_BASE_URL + "/toggletotp", objData);
...
export async function handlePostChangePasswordFlow(flowID, csrf_token, password) {
  try {
    ...
    const res = await axiosInstance.post(process.env.NEXT_PUBLIC_BASE_URL + "/changepassword", objData);
    return res.data.status;
  } catch (error) {
    throw error;
  }
...
export async function handlePostUpdateProfileFlow(flowID, csrf_token, traits) {
  try {
    ...
    const res = await axiosInstance.post(process.env.NEXT_PUBLIC_BASE_URL + "/updateprofile", objData);
    return res.data.status;
  } catch (error) {
    throw error;
  }
