...
export async function handleGetRegisterFlow() {
  try {
    const getResponse = await axiosInstance.get(process.env.NEXT_PUBLIC_BASE_URL + "/register");
...
export async function handlePostRegisterFlow(flowID, csrf_token, password, email, name, phone_number) {
  try {
    ...
    const res = await axiosInstance.post(process.env.NEXT_PUBLIC_BASE_URL + "/register", objData);
    return res;
  } catch (error) {
    throw error;
  }
